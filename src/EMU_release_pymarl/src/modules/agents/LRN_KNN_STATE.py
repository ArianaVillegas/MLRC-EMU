...
class LRU_KNN_STATE:
    ...
    def aumento_de_penalizacion(self, ind):
        ...
        if self.Ncall[ind] > self.umbral_visitas:
            ...
            penalizacion = self.penalization_manager.penalizacion_sigmoide(self.Ncall[ind])
            return penalizacion
        ...
        if np.allclose(self.states_norm[ind], key_norm, rtol=self.rtol, atol=self.atol ):
            ...
            if modify:
                ...
                if (self.xi[ind] == 0) and (xit == 1) and self.use_AEM : # memory shift
                    ...
                    if value_decay + Nuevo_incentivo > self.q_values_decay[ind]: 
                        self.q_values_decay[ind] = value_decay+ Nuevo_incentivo
"""
                    Nuevo_incentivo = self.aumento_de_penalizacion(ind)
